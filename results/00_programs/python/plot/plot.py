...
#############
# generate plot
def gen_plot (fig = None, linewidth = default_plot_linewidth, markersize = default_plot_markersize, legendloc = default_legendloc, fit = False, show = True, save = True):
    ...
    if fit is not None:
        ...
        if type(fit) is list:
            ...
            line = plt.plot(fit.get_xval_list(lims = xlim, n = n_xfits, log = fig.xaxis_is_logscale()), fit.get_yval_list(lims = xlim, n = n_xfits, log = xaxis_is_logscale), linewidth = fit.get_linewidth(), marker = fit.get_marker(), markersize = fit.get_markersize(), ls = fit.get_linestyle(), color = fit.get_linecolor())
