...
declare -i N_PERIOD_EQUILIBRIUM=500000
...
declare -i MAX_PERIOD_VAL=100000000 # 100 million
# minimum period value to test
declare -i MIN_PERIOD_VAL=10000 # 10 thousand
# force base value
VAL_FO="0.0"
# force amplitude
VAL_FA=( "0.5" "5.0" )
...
MAX_SIMULATION_MCS=100000000000 # 100 billion
...
gen_simparm() {
	...
	HEADER_SIMPARM="id,path,pot,MCS,N,R,k,fo,fA,T,fv"
...
# 			fi
			...
			for c in ${PARM_CSA[@]}; do
				...
				for l in "${PARM_LP[@]}"; do
					...
					## flags
					## directory (potentiall generated by script depending on parameters)
					## execute script
					## slurm script
					## simulation id (potentially generated by script depending on parameters)

					for fa in "${VAL_FA[@]}"; do

						# generate simulation id and directory
						SIMDIR="${R}/${C}/N${n}/LP${l}/NF/"
						...
						echo "${SIMID},${SIMDIR},${C},100000000,${n},${r},${K_STRING},${VAL_FO},${fa},0.,${FORCEVEC}" >> $FILE_SIMPARM
						...
						for p in $(seq 0 $(($N_PERIOD_VAL-1))); do
							...
							SIMDIR="${R}/${C}/N${n}/LP${l}/FO${VAL_FO}/FA${fa}/T${PERIOD_VAL}/FV${FORCEVEC}/"
							SIMID="${R}${C}_N${n}LP${l}_FV${FORCEVEC}FO${VAL_FO}FA${fa}T${PERIOD_VAL}"
							...
							GENFLAGS="-n ${n} -v ${FORCEVEC} -f ${VAL_FO} -a ${fa} -p ${PERIOD_VAL} -b 512 ${R_FLAG}${K_FLAG}${C_FLAG}"
							...
							./00_programs/bash/gen_LeMonADE_exec.sh ${PATH_SIMPARM}${SIMDIR} ${SIMID} "${GENFLAGS}" "${SIMFLAGS}"

							# add parameters to parm file
							# "id,path,pot,N,R,k,fo,fA,T,fv"
							echo "${SIMID},${SIMDIR},${C},${TOTAL},${n},${r},${K_STRING},${VAL_FO},${fa},${PERIOD_VAL},${FORCEVEC}" >> $FILE_SIMPARM
